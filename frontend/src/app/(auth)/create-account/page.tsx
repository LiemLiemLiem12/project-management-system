...
export default function CreateAccountPage() {
  ...
  const [isPending, setIsPending] = useState(false); // Thay bằng pending state từ service của bạn

  const { completeSignup } = useAuthService(); // Hook API của bạn
    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-10 bg-gray-50/50">
      <div className="form-container flex flex-col items-center w-full max-w-[550px] gap-8 bg-white rounded-2xl px-8 md:px-12 py-12 shadow-2xl border border-gray-100">
        ...
        <div className="w-full space-y-4">
          ...
          {/* Username & Birthday Row (Chia 2 cột trên màn hình vừa trở lên) */}
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="space-y-1 flex-1">
              ...
              {/* Note: Tùy thuộc vào component TextInput của bạn có hỗ trợ type="date" tốt không, 
                  nếu không bạn có thể dùng thẻ <input> thường với class tương tự */}
              <TextInput
                id="birthday"
                type="date"
        ...
        <button
          ...
          disabled={isPending}
          className="w-full cursor-pointer flex justify-center items-center bg-primary hover:bg-primary-dark text-white py-3 rounded-full font-bold text-lg transition-all shadow-lg shadow-blue-100 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed mt-2"
        >

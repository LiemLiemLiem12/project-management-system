...
export const handleImageUpload = async (
  file: File,
  uploadMedias: (body: FormData) => Promise<any>,
  ...
  // Khởi tạo FormData và append file với key là "files" (giống với FilesInterceptor bên backend)
  ...
  try {
    ...
    const responseData = await uploadMedias(formData);

    let uploadedUrl = responseData[0].file_url;

    onProgress?.({ progress: 100 });

    if (!uploadedUrl) {
      toast.error("Can not get Image");
    }
